   ...
   topo_path = '/zulu/nmolem/DATASETS/TOPO/';

%  gridfile = '/avatar/nmolem/SFBAY/sfbay_grd.nc';
%  gridfile = '/zulu/nmolem/EBMUD/ebmud_grd.nc';
   gridfile = '/zulu/nmolem/SMB/smb_grd.nc';
%  Make 
%  clear jet
%  colormap(jet(256))
...
%    climat = 'sfbay_cen_1m';
   ...
   if strcmp(lower(climat),'crm_west')  & ~formask
    ncwriteatt(gridfile,'hraw','source',['Raw bathymetry from Coastal Relief model at 90m (NOAA) (Smoothing radius ' num2str(wd) ' )']);
   end
   if strcmp(lower(climat),'socal_30m')  & ~formask
    ncwriteatt(gridfile,'hraw','source',['Raw bathymetry from Coastal Relief model at 30m (NOAA) (Smoothing radius ' num2str(wd) ' )']);
   end
   if strcmp(lower(climat),'sfbay_10m')  & ~formask
    ncwriteatt(gridfile,'hraw','source',['Raw bathymetry from 10m DEM for SF bay (cnra.ca.gov) (Smoothing radius ' num2str(wd) ' )']);
   end
...
%    end
     ...
     switch lower(climat);
      ...
      case{'crm_west'}
        disp('Using CRM_west')
        datafile = [topo_path 'crm_west.nc']; %% from 
        topo_lon = ncread(datafile,'x');
        topo_lat = ncread(datafile,'y');
...
%       topo_lat = nct{'y'}(:);
	...
	topo_lat = topo_lat(1,:);
      ...
      case{'socal_30m'}
        ...
        datafile = [topo_path 'socal_30m.nc'];
        topo_lon = ncread(datafile,'lon');
        topo_lat = ncread(datafile,'lat');
        topo_lon = topo_lon + 360;
     end
