...
% disp('tsearchn done')
    tt = delaunayTriangulation(xp(:),yp(:));
  [tn,pn] = pointLocation(tt,Xc);
  disp('pointLocation done')

  % tt.ConnectivityList equals tri from tri = delauney(xp(:),yp(:));
  % tt.Points   equals xp,yp from tri = delauney(xp(:),yp(:));
