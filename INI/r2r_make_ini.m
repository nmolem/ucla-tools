...
%     size(hs)
    % Z-coordinate (3D) on minimal subgrid and child grid
      zs = zlevs3(hs, hs*0, theta_s_p, theta_b_p, hc_p, N_p, 'r','new2008');
      zc = zlevs3(hc, hc*0, theta_s_c, theta_b_c, hc_c, N_c, 'r','new2008');
      zw = zlevs3(hc, hc*0, theta_s_c, theta_b_c, hc_c, N_c, 'w','new2008');
...
%     save 'testing' 'A' 'var'
      ...
      var = fillmask(var,1,masks,nnel);
      var = double(var);
      ini_p_sl = reshape(A*reshape(var,Np*Mp*Lp,1),Nc,Mc,Lc);
    ...
    % Read in staggered velocities
      ...
      ud_sl = squeeze(ncread(par_data,'u_slow'  ,[imin jmin 1 tind],[imax-imin,jmax-jmin+1 inf 1]));
      vd_sl = squeeze(ncread(par_data,'v_slow'  ,[imin jmin 1 tind],[imax-imin+1,jmax-jmin inf 1]));

      ud(abs(ud)>10) = 0;
      vd(abs(vd)>10) = 0;
      ud_sl(abs(ud_sl)>10) = 0;
      vd_sl(abs(vd_sl)>10) = 0;
    ...
    % Average to rho points
      ...
      ur = permute(ur,[3 2 1]);
      vr = permute(vr,[3 2 1]);
      ur_sl = permute(ur_sl,[3 2 1]);
      vr_sl = permute(vr_sl,[3 2 1]);
    ...
    % Rotate to north
      us = zeros(Np,Mp,Lp);
      vs = zeros(Np,Mp,Lp);
      us_sl = zeros(Np,Mp,Lp);
      vs_sl = zeros(Np,Mp,Lp);
      size(sins)
      size(ur)
      size(coss)
      size(vr)
      for k = 1:Np
        ...
        us_sl(k,:,:) = squeeze(ur_sl(k,:,:)).*coss - squeeze(vr_sl(k,:,:)).*sins;
        vs_sl(k,:,:) = squeeze(vr_sl(k,:,:)).*coss + squeeze(ur_sl(k,:,:)).*sins;
      end

      us = fillmask(us, 0, masks, nnel);
      vs = fillmask(vs, 0, masks, nnel);
      us_sl = fillmask(us_sl, 0, masks, nnel);
      vs_sl = fillmask(vs_sl, 0, masks, nnel);
      us = double(us);
      vs = double(vs);
      us_sl = double(us_sl);
      vs_sl = double(vs_sl);
    ...
    % Rotate to child orientation
      us = zeros(Nc, Mc, Lc);
      vs = zeros(Nc, Mc, Lc);
      us_sl = zeros(Nc, Mc, Lc);
      vs_sl = zeros(Nc, Mc, Lc);
      for k=1:Nc
        us(k,:,:) = squeeze(ud(k,:,:)).*cosc + squeeze(vd(k,:,:)).*sinc;
        vs(k,:,:) = squeeze(vd(k,:,:)).*cosc - squeeze(ud(k,:,:)).*sinc;
        us_sl(k,:,:) = squeeze(ud_sl(k,:,:)).*cosc + squeeze(vd_sl(k,:,:)).*sinc;
        vs_sl(k,:,:) = squeeze(vd_sl(k,:,:)).*cosc - squeeze(ud_sl(k,:,:)).*sinc;
      end
    % Back to staggered locations
      u = 0.5*(us(:,:,1:Lc-1) + us(:,:,2:Lc)); 
      v = 0.5*(vs(:,1:Mc-1,:) + vs(:,2:Mc,:));
      u_sl = 0.5*(us_sl(:,:,1:Lc-1) + us_sl(:,:,2:Lc)); 
      v_sl = 0.5*(vs_sl(:,1:Mc-1,:) + vs_sl(:,2:Mc,:));
    ...
    % Zero-ing out the mask
      for k = 1:Nc
        ...
        ini_p_sl(k,:,:) = squeeze(ini_p_sl(k,:,:)).*maskc;
        u(k,:,:)        = squeeze(u(k,:,:)).*umask;
        v(k,:,:)        = squeeze(v(k,:,:)).*vmask;
        u_sl(k,:,:)     = squeeze(u_sl(k,:,:)).*umask;
        v_sl(k,:,:)     = squeeze(v_sl(k,:,:)).*vmask;
      end
      ...
      ini_p_sl = permute(ini_p_sl,[3 2 1]);
      ncwrite(chd_data,'p_slow'  ,ini_p_sl,[icb jcb 1 1]);
      u = permute(u,[3 2 1]);
      ncwrite(chd_data,'u'     ,u       ,[icb jcb 1 1]);
      v = permute(v,[3 2 1]);
      ncwrite(chd_data,'v'     ,v       ,[icb jcb 1 1]);
      u_sl = permute(u_sl,[3 2 1]);
      ncwrite(chd_data,'u_slow'     ,u_sl       ,[icb jcb 1 1]);
      v_sl = permute(v_sl,[3 2 1]);
      ncwrite(chd_data,'v_slow'     ,v_sl       ,[icb jcb 1 1]);
%     w = permute(w,[3 2 1]);
